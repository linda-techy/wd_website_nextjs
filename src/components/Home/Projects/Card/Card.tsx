...
const PropertyCard: React.FC<{ item: PropertyHomes }> = ({ item }) => {
  const { name, location, beds, baths, area, slug, images, status } = item
      ...
      <div className="relative rounded-2xl border border-dark/10 dark:border-white/10 group hover:shadow-3xl duration-300 dark:hover:shadow-white/20">
        ...
        <div className="p-3 sm:p-4">
          <div className="flex flex-col mobile:flex-row gap-2 mobile:gap-0 justify-between mb-3">
            <div>
              <Link href={`/properties/${slug}`}>
                <h3 className="text-lg sm:text-xl md:text-2xl font-semibold text-black dark:text-white duration-300 group-hover:text-primary leading-tight">
                  {name}
                </h3>
              </Link>
            ...
            <div className="self-start">
              <button className={`text-xs sm:text-sm font-semibold px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full whitespace-nowrap ${getStatusClasses(status)}`}>
                {status}
              </button>
            </div>
          </div>
